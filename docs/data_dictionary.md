...
* パース不能は、エラーではなく `unknown` とし、補正対象から除外します。

#### パス形式の前提と例外

本プラグインは、`_wp_attached_file` が `yyyy/mm/...` 形式であることを前提とします。

#### 非対応ケース

以下の場合は、`unknown` とします。

* 年月フォルダーが、(`uploads/image.jpg` など) 無効
* 先頭に `yyyy/mm` が存在しない
* カスタムディレクトリ構成
...
| 状態 | 条件 |
| ---------- | ------------------------------------------- |
| `unknown` | `pathYm === null` または `postDateYm === null` のいずれかに該当すること |
| `match` | 両方が存在し、年月が一致すること |
| `mismatch` | 両方が存在し、かつ年月が一致しないこと |
...
本ドキュメントの型定義は、OpenAPI にもとづき、生成または整合されます。

OpenAPI が、最上位の契約となります。
