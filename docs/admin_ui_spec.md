...
WordPress 標準の UI 構造に準拠し、ユーザーの認知負荷を低減します。
...
* メディアの並び替えは、既存のメディアライブラリ画面に委ねます。
  * [Intuitive Custom Post Order](https://ja.wordpress.org/plugins/intuitive-custom-post-order/) などのプラグインとの互換性を維持します。
...
* [冪等性 (べきとうせい)](./architecture.md#冪等性-べきとうせい) を前提に、複数回実行しても結果が変わらないよう設計します。
...
本テーブルは、WordPress 標準のメディア一覧とは独立した表示であり、メディアの並び替えは、既存のメディアライブラリ画面に委ねます。

* [Intuitive Custom Post Order](https://ja.wordpress.org/plugins/intuitive-custom-post-order/) などのプラグインとの互換性を維持します。

### 並び順と処理の関係

一覧の並び順は、表示上のものであり、補正処理の結果には影響しません。
...
React UI として表示される本テーブルでは、上記「既存カラム」の他に、論理カラムとして、下記を追加します。
...
差分判定に関する補足として、下記を挙げます。

* 日 (dd) は、比較対象としません。
* 時刻も、無視します。

表示仕様は、次のとおりです。ただし、WAI-ARIA の観点から、色だけで差分を提示してはなりません。
...
* 現在のフィルター結果に対して、全件適用する方式 (推奨)。
* 全メディアに対して適用する方式 (非推奨・要確認)。

本プラグインでは、以下を採用します。

* 下記の理由から、現在の一覧 (検索・フィルター結果) を対象とします。
  * WordPress 標準の挙動に準拠するためです。
  * 意図しない全件更新を防止するためです。
