...
本画面での「一覧」は、REST API により取得したメディア一覧を指します。

Bulk Action の対象は、下記です:
...
テーブルは、WordPress 標準の `WP_List_Table` 構造に準拠し、上記の既存カラムに加えて拡張カラムを追加します。

## カラム定義

追加カラム (React UI 上のテーブルに表示される論理カラム) は、以下の通りです。
