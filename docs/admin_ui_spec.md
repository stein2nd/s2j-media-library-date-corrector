...
本画面では、未処理要素 `not_processed` を REST API から直接受け取らず、`summary` の差分から推定します。
...
* REST API のエラーコードは、翻訳せず、UI 側でマッピングします。
* 将来的な多言語対応を前提とします。
...
* 「Date Correct (All)」は、選択状態に依存せず、現在のフィルター結果全体に対して適用されます。
...
* 一致する場合は MATCH とし、正常を示します。
* 不一致の場合は MISMATCH とし、補正対象を示します。
