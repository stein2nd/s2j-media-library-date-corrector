...
| ヘッダー | 必須 | 説明 |
|--------|------|------|
| `X-WP-Nonce` | ログインユーザー操作時は **必須** | `wp_create_nonce('wp_rest')` の値です。`api-fetch` はデフォルトで付与します。 |
| `Content-Type` | POST/PUT 時 | `application/json` とします。 |
...
| レイヤー | 内容 |
| -------- | ---------------------------- |
| HTTP レベル | `upload_files` を満たさない場合は、`403` |
| 件別処理レベル | `edit_post` により、個別判定 |
...
以下の場合は、リクエスト全体を拒否し、HTTP ステータス `401` または `403` を返し、「リクエスト自体が成立しない」ケースとして扱います。
...
* バッチ処理において、実用性に優れています。
* WordPress REST API の一般的な運用に準拠します。
* UI における「成功件数/失敗件数」の表示や再試行に適しています。
