...
本 REST API 仕様は、OpenAPI (Swagger) 形式で表現可能とします。
...
エンドポイント、リクエスト構造、レスポンス構造、エラー定義です。
...
詳細は、[データ辞書](./data_dictionary.md#typescript-型定義-完全版) をご覧ください。
...
* UI は、`total` を基準に、ページネーションを構築します。

#### 注意

* `total` は、ページネーションとは、独立しています。
* フィルター変更時に変動します。

#### total の定義

`total` は、現在のフィルター条件に一致する件数を表します。
...
`unknown` 状態の要素は、補正対象外とします。

* 実行時は、`skipped` として扱います。
  * 補正の実行に必要な情報が、不足しているためです。
...
デバッグ用途として、必要に応じて、未処理要素 (`not_processed`) を返すオプションを追加可能とします。

```text
?includeNotProcessed=true
```

また、必要に応じて、専用の retry エンドポイントを追加可能とします。
...
| 操作 | 条件 (想定) |
|------|----------------|
| 一覧・プレビュー系 | 各 `attachment` ID について `current_user_can('edit_post', $id)` が成り立つ必要があります。 |
...
リクエストで指定される `ids` は、以下の条件を満たすものとします。
...
本 API は、フィルター条件を解釈しません。

* フィルター条件は、UI 層のみが管理します。
* API は、ID リストのみを受け取ります。
...
* **REST エラー (`WP_Error` 相当)**
  * レスポンス全体が失敗するときの `code` です (たとえば、`invalid_param`、`permission_denied`)。HTTP は `4xx`/`5xx` となります。
* **件別 `results[].message`**
